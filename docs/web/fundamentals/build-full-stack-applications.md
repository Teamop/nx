...
tao g @nrwl/web:application frontend # Create a Web Components Application
```
...
- `tao serve frontend` to serve the application
- `tao build frontend` to build the application
- `tao test frontend` to test the application
...
tao g @nrwl/express:application api --frontend-project frontend # sets up the proxy configuration so you can access the API in development
```
...
Now, run `tao serve frontend & tao serve api`, and open [http://localhost:4200](http://localhost:4200) to see both the frontend and backend working:
...
```bash
tao g @nrwl/workspace:library data # This generates a barebone library with only Typescript setup
```
